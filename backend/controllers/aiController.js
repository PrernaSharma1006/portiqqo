    ...
    const groqRes = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Portfolio data:\n${JSON.stringify(portfolioData, null, 2)}\n\nCommand: "${command}"\n\nRules:\n1. Return ONLY raw JSON — no markdown, no explanations\n2. Keep ALL existing fields intact unless the command changes them\n3. For color/style changes: add or update the "customStyles" object with CSS color values e.g. { "customStyles": { "nameColor": "#7c3aed", "headingColor": "#1e1b4b" } }\n4. For THEME changes ("switch to dark mode", "make it dark", "light theme", "minimal", "colorful"): set customStyles.theme to one of: "dark", "light", "minimal", "colorful"\n5. For FONT SIZE changes ("make headings larger", "bigger text", "smaller headings"): set customStyles.headingSize to one of: "small", "medium", "large", "xlarge"\n6. For FONT FAMILY changes ("use serif font", "use monospace", "use Roboto"): set customStyles.headingFont and/or customStyles.bodyFont to a valid CSS font-family string\n7. For BULK/VAGUE style commands ("make it more professional", "make everything minimal", "make it look modern"): choose the most fitting theme + individual style tweaks\n8. For section visibility: update "hiddenSections" array (add to hide, remove to show)\n9. For adding/editing content: update the relevant field or append to the relevant array\n10. customStyles keys available: theme, pageBg, sectionAltBg, nameColor, headingColor, titleColor, cardBg, accentColor, bodyText, headingFont, bodyFont, headingSize\n\nReturn the complete updated JSON object only.`
          }
        ],
        temperature: 0.3,
        max_tokens: 4096
